...
board.on('ready', function() {
    ...
    var pidUtils = {
        ...
        pidLoop: function(motion) {
            ...
            ctrl.compute();
            console.log(motion+','+new Date().getTime()+','+ctrl.getOutput()+','+(setDegree - pid_heading)+','+0+','+setDegree+','+heading);
            if (motion === 'forward') {
    ...
    compass.once('data', function() {
        ...
        pidUtils.init();
        picTick1 = setInterval(function() {
            pidUtils.pidLoop('forward');
        }, 5);
        setTimeout(function() {
            clearInterval(picTick1);
            motorUtils.Stop();
            ctrl.setOutputLimits(15, 255);
            setDegree = setDegreeLimit(setDegree + 140);
            ctrl.setTunings(10, 0, 1);
            var picTick2 = setInterval(function() {
                pidUtils.pidLoop('rotate');
            }, 5);
            var picTick3 = setInterval(function() {
                if (Math.abs(setDegree - heading) < 1) {
                    clearInterval(picTick2);
                    clearInterval(picTick3);
                    motorUtils.Stop();
                }
            }, 5);
        }, 10000);
